...
const ChromaticSmoke = () => {
  ...
  return (
    <div 
      ref={containerRef} 
      className="fixed inset-0 w-full h-full bg-ai-background"
      style={{ 
        zIndex: -1,
        background: `
          radial-gradient(circle at 25% 25%, hsl(var(--ai-accent-glow) / 0.1) 0%, transparent 50%),
          radial-gradient(circle at 75% 75%, hsl(var(--ai-matrix) / 0.08) 0%, transparent 50%),
          linear-gradient(135deg, hsl(var(--ai-background)) 0%, hsl(var(--ai-background-secondary)) 100%)
        `
      }}
    />
  );
